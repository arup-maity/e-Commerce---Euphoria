...
const DeleteUser: React.FC<PropsType> = ({ open, setShowDelete, selectedUser }) => {
   const defaultValues = {
      fullName: '',
      email: '',
      role: ''
   ...
   const onSubmit: SubmitHandler<Inputs> = async (data) => {
      try {
         const response = await adminInstance.delete(`/user/delete-user/${selectedUser.id}`)
   ...
   return (
      <div>
         <Offcanvas isOpen={open} onOpen={handleOpen} toggle={() => setShowDelete(!open)}>
            <Offcanvas.Header toggle={() => setShowDelete(!open)}>
            ...
            <Offcanvas.Body>
               <form onSubmit={handleSubmit(onSubmit)} className='w-full h-full flex flex-col'>
                  ...
                  <div className="flex items-center gap-2">
                     <button type='submit' className='basis-1/2 text-base bg-red-400 text-white border border-red-400 rounded py-1 px-4'>Delete</button>
                     <button onClick={() => setShowDelete(!open)} className='basis-1/2 text-base bg-gray-200 text-gray-600 border border-gray-200 rounded py-1 px-4'>Cancel</button>
                  </div>
               </form>
            </Offcanvas.Body>
         </Offcanvas>
